...
WantedBy=multi-user.target"""
    ...
    def build_install_plan(self) -> List[Tuple[str, str]]:
        """
        Формирует полный план установки для режима dry-run.

        План включает все реальные команды, которые будут выполнены на сервере,
        а также техническую паузу перед проверкой статуса сервиса.

        Returns:
            List[Tuple[str, str]]: Список шагов в формате (описание, команда).
        """
